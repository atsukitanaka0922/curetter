// components/UserPreview.jsx - 他ユーザー向けプレビューコンポーネント（エピソード表示改善版）
'use client'
...
export default function UserPreview({ userId, onClose }) {
  ...
  const [episodeTypesData, setEpisodeTypesData] = useState([]) // エピソード詳細データ用
  ...
  useEffect(() => {
    if (userId) {
      loadUserData()
      getEpisodeTypesData() // エピソード詳細データを取得
    }
  }, [userId])
  ...
  const formatEpisodeDisplay = (episodeName) => {
    ...
    if (episodeDetails) {
      return {
        fullText: `【${episodeDetails.category}】第${episodeDetails.episode_number}話 ${episodeName}`,
