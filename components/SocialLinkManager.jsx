...
export default function SocialLinkManager({ links = [], onLinksChange }) {
  ...
  const platforms = [
    ...
    { 
      id: 'Discord', 
      name: 'Discord', 
      placeholder: 'https://discord.gg/invite または discord.com/users/123456',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
    ...
    { 
      id: 'ニコニコ動画', 
      name: 'ニコニコ動画', 
      placeholder: 'https://www.nicovideo.jp/user/12345',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
  ...
  const validatePlatformUrl = (platform, url) => {
    const validators = {
      'X (Twitter)': (url) => url.includes('x.com') || url.includes('twitter.com'),
      'YouTube': (url) => url.includes('youtube.com') || url.includes('youtu.be'),
      'pixiv': (url) => url.includes('pixiv.net'),
      'Instagram': (url) => url.includes('instagram.com'),
      'TikTok': (url) => url.includes('tiktok.com'),
      'Twitch': (url) => url.includes('twitch.tv'),
      'Discord': (url) => url.includes('discord.gg') || url.includes('discord.com'),
      'ニコニコ動画': (url) => url.includes('nicovideo.jp') || url.includes('nico.ms')
    }

    if (platform === 'その他') return true
    return validators[platform] ? validators[platform](url) : true
  }
  ...
  const addLink = () => {
    ...
    if (!validateUrl(newLink.url)) {
      setError('有効なURLを入力してください（http://またはhttps://で始まる必要があります）')
      return
    }

    if (!validatePlatformUrl(newLink.platform, newLink.url)) {
      setError(`${newLink.platform}の正しいURLを入力してください`)
      return
    }
    ...
    // フォームリセット
    setNewLink({ platform: '', url: '', display_name: '' })
  ...
  const getPlatformIcon = (platformName) => {
    ...
    return platform ? platform.icon : <Globe className="w-4 h-4" />
  }

  // プラットフォームカラー取得
  const getPlatformColor = (platformName) => {
    const platform = platforms.find(p => p.id === platformName)
    return platform ? platform.color : 'text-gray-700'
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={
                  newLink.platform 
                    ? platforms.find(p => p.id === newLink.platform)?.placeholder || 'https://example.com'
                    : 'https://example.com'
                }
              />
            </div>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        ...
        <p>• 最大8個まで追加可能です</p>
        <p>• 表示名を設定しない場合はプラットフォーム名が表示されます</p>
        <p>• リンクは新しいタブで開かれます</p>
      </div>
    </div>
  )
